...
public class PgResultSetMetaData implements ResultSetMetaData, PGResultSetMetaData {
   ...
   * @exception SQLException if a database access error occurs
   */
  public int getColumnDisplaySize(int column) throws SQLException { // pg_type_size
    Field field = getField(column);
...
*/
    ...
    StringBuilder sql = new StringBuilder(
        "SELECT * from ("
        + " SELECT t.oid, c.cid as attnum, c.name as attname, c.table as relname, c.schema as nspname, c.notnull as attnotnull, 0 as autoinc"
        + " FROM sys.sys_columns as c"
        + " JOIN (SELECT xdmp_hash64(name) as oid, name from sys.sys_tables) t ON (t.name = c.table)"
        + " ) as ml"
        + " JOIN ("); // INNER JOIN with existing field data oid and attnum (table# and column#)

/* 0 as oid seems unlikely to work unless it means table_cat so I made a hash64 of table name */
/* int table (oid), int column (attnum), colum_name (attname), table_name (relname), schema_name (nspname), int nullable (attnotnull), boolean autoincrement */
    ...
    sql.append(") vals ON (ml.oid = vals.oid AND ml.attnum = vals.attnum) ");
...
//(rs.getInt("attnotnull") != 0)
        int nullable =
            (rs.getInt(6) != 0) ? ResultSetMetaData.columnNoNulls : ResultSetMetaData.columnNullable;
